...
static void usart_isr (struct rt_serial_device *serial)
{
    ...
    if (usart_interrupt_flag_get(uart->config->periph, USART_INT_FLAG_RBNE) != RESET)
    {
        ...
        rt_ringbuffer_putchar(&(rx_fifo->rb), usart_data_receive(uart->config->periph));
    ...
    else if (usart_interrupt_flag_get(uart->config->periph, USART_INT_FLAG_TBE) != RESET)
    {
        ...
        {
            usart_data_transmit(uart->config->periph, put_char);
        }
        else
        {
            usart_interrupt_disable(uart->config->periph, USART_INT_TBE);
            usart_interrupt_enable(uart->config->periph, USART_INT_TC);
        }
        usart_interrupt_flag_clear(uart->config->periph, USART_INT_FLAG_TBE);
    }
    else if (usart_interrupt_flag_get(uart->config->periph, USART_INT_FLAG_TC) != RESET)
    {
        usart_interrupt_disable(uart->config->periph, USART_INT_TC);

        rt_hw_serial_isr(serial, RT_SERIAL_EVENT_TX_DONE);

        usart_interrupt_flag_clear(uart->config->periph, USART_INT_FLAG_TC);
    }
...
static rt_size_t gd32_transmit (struct rt_serial_device *serial, rt_uint8_t *buf, rt_size_t size, rt_uint32_t tx_flag)
{
    struct gd32_uart *uart;
